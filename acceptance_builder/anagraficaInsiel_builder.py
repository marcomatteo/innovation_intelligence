...
class AnagraficaInsielBuilder(AcceptanceBuilder):

    def __init__(self, inTest:bool=False):
        self.dp = AnagraficaInsiel(inTest=inTest)
        if not self.dp.is_preprocessed("Cessazione artigiana"):
            self.dp.update_artisan("Cessazione artigiana", save=False)
        # self.dp.df = self.dp.df.iloc[:,  list(self.dp.column_constraints.keys())]
        self.dp_file_extension = "xlsx"
        self.column_number = 50
        self.columns = [
            ...
            ,AcceptanceBuilder.Columns(nome='Tipo Localizzazione', tipologia = np.dtype('O'), lunghezza=999, nullable=True, pk=False)
        ]
